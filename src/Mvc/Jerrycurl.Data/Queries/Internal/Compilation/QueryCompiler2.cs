...
namespace Jerrycurl.Data.Queries.Internal.Compilation
{
    internal class QueryCompiler2
    {
        ...
        public ListFactory Compile(ListResult result)
        {
            ...
            foreach (ListTarget2 target in result.Targets2)
            {
        ...
        #region " Prepare "
        private Expression GetPrepareVariableExpression(ListTarget2 target)
        {
        ...
        private Expression GetPrepareBufferExpression(ListTarget2 target)
        {
        ...
        private Expression GetKeyInitializeBufferExpression(JoinTarget2 join)
        {
            ...
            Expression addArray = this.GetDictionaryAddExpression(join.Buffer, join.Key.Variable, assignBuffer);
        ...
        private Expression GetKeyBlockExpression(IEnumerable<KeyReader> primaryKeys, IEnumerable<JoinTarget2> joins, Expression body)
        {
            ...
            foreach (JoinTarget2 join in joins.NotNull())
            {
        ...
        private Expression GetTryCatchExpression(BaseReader reader, Expression expression)
        {
            ...
            MethodInfo constructor = typeof(QueryCompiler).GetStaticMethod(nameof(QueryCompiler2.GetInvalidCastException), typeof(ISchema), typeof(string), typeof(Exception));
