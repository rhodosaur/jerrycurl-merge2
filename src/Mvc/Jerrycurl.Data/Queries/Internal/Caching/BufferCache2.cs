...
{
    internal class BufferCache2
    {
        private readonly Dictionary<BufferCacheKey2, int> parentMap = new Dictionary<BufferCacheKey2, int>();
        ...
        public BufferCache2(ISchema schema)
        {
        ...
        public int GetListIndex(MetadataIdentity target) => this.GetParentIndex(new BufferCacheKey2(target));
        public int GetParentIndex(IReference reference)
        {
            IEnumerable<Type> key = this.GetParentKeyType(reference);
            BufferCacheKey2 cacheKey = new BufferCacheKey2(key);
        ...
        private int GetParentIndex(BufferCacheKey2 key)
        {
            lock (this.state)
